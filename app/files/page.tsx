...
export default function FilesPage() {
  ...
  return (
    <div className="max-w-6xl m-4 sm:m-10 flex flex-col gap-8 grow items-stretch">
      <div className="h-40 flex flex-col justify-center items-center border-b border-primary/20 pb-8">
        ...
        <Input
          type="file"
          name="file"
          className="cursor-pointer w-full max-w-xs border-primary/30 hover:border-primary/60 transition-colors"
          onChange={async (e) => {
      ...
      {documents && (
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
          {documents.map((document) => (
            <div
              key={document.id}
              className="flex flex-col gap-2 justify-center items-center border border-primary/20 rounded-md p-4 sm:p-6 text-center overflow-hidden cursor-pointer hover:bg-primary/5 transition-colors"
              onClick={async () => {
            ...
            >
              ...
              <span className="text-foreground mt-2">{document.name}</span>
            </div>
          ))}
        </div>
      )}
